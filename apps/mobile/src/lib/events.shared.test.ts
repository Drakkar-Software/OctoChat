...
// `events.shared` value-imports server URLs from `./starfish/config` (used by
// subscribeRoomChanges, not by the pure parseRoomChange under test). Stub them so
// the module loads under Node without pulling in the Expo env.
vi.mock('./starfish/config', () => ({ EVENTS_URL: 'http://x/events', SYNC_BASE: 'http://x' }));
