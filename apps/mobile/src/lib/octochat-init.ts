...
export function initOctoChat(): void {
  ...
  // heavy native-module wrapper; lazy-import it on first invocation so it doesn't
  // evaluate during boot (automations rarely fire on cold start). `configureLlm`
  // stores the thunk; the SDK calls it only when a tick actually needs generation.
  // Web leaves the port unconfigured so AI automations report "not available on
  // this device" rather than silently posting empty output.
  if (Platform.OS !== 'web') {
    configureLlm((messages, opts) =>
      import('@/lib/ai/llm-adapter').then((m) => m.generateText(messages, opts)),
    );
  }
}
