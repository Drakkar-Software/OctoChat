...
// `threads` → `message-view` → `links` value-imports `expo-router` + `react-native`
// for its navigation helpers (unused here). Stub them so the pure helper runs under
// Node, mirroring links.test.ts.
