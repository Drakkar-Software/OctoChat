...
export function AppFrame({ children }: { children: ReactNode }) {
  ...
  // below the banner while it's showing: the native nav header keys its status-bar
  // strip off this same inset (`topInset !== 0`), so header + content stay in sync
  // and the empty band under the banner disappears. (Web places the banner above
  // the bottom bar, so no top inset is consumed there — nothing to zero.)
